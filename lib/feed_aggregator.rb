...
class Feed
    ...
    def get_xml_builder(feed=@feed, type=@type, time=@time)
        builder = Nokogiri::XML::Builder.new("encoding" => "UTF-8") { |xml|
            xml.opml('version' => "1.1"){
                ...
                xml.body{
                    feed.entries.each do |entry|
                        xml.outline("title"=>entry.title, "text"=>entry.title, "type"=>type, "xmlUrl"=>entry.url)
                    end
                }
            }
        }
    ...
    def write_opml(save_path="")
        ...
        File.write(file_name, xml_builder.to_xml)
    end
